...
use super::{types::*};
use crate::core::io::{ExecInput, ExecOutput};
...
macro_rules! exec_fn {
  (|$t:ident, $core:ident| $body:block) => {{
    use crate::core::schema::ExecuteFn;

    ExecuteFn::Sync(|$t, $core| $body)
  }};
  (|$t:ident| $body:block) => {{
    use crate::core::schema::ExecuteFn;

    ExecuteFn::Sync(|$t, _| $body)
  }};
  (|$t:ident, $core:ident| async $($body:tt)*) => {{
    use crate::core::schema::ExecuteFn;

    ExecuteFn::Async(|$t, $core| Box::pin(async move {
      async $($body)*.await
    }))
  }};
}
